 ...
 *              本迁移以幂等方式将 7 个 description 列收敛为最终类型 VARCHAR(2000)，
 *              保证任意起点的库在应用完全部迁移后 schema 完全一致。
 *              今后列类型变更一律新增前向迁移，禁止回改历史迁移文件。
 *
 *              执行流程（与 013 同构，幂等安全）：
 *              1. 已是 VARCHAR(2000) 的列跳过
...
export const migration: Migration = {
  ...
  async up(dbPool: Pool) {
    ...
    for (const col of DESCRIPTION_COLUMNS) {
      ...
      if (info.DATA_TYPE === "varchar" && Number(info.CHARACTER_MAXIMUM_LENGTH) === MAX_LEN) continue;
      ...
      console.log(`[migration-032] ${col}: ${info.DATA_TYPE} → VARCHAR(${MAX_LEN})…`);
      await dbPool.query(
