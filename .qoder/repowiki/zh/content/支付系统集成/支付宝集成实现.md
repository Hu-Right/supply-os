...
## 更新摘要
**所做更改**   
- 更新了支付宝提供商文件路径以适配新的模块结构
- 调整了导入路径引用，从 src/payment 迁移到 src/core/payment
- 更新了环境检测器的文件路径引用
- 保持了文档的整体结构和一致性
...
- 前端支付模块位于 src/payment，包含支付宝、微信、Mock 提供者及统一支付服务。
- 核心支付配置和环境检测位于 src/core/payment，提供环境识别和配置管理。
...
- index.ts
  - 提供核心支付模块的统一导出，简化导入路径。

**更新** 环境检测器已从 src/payment/env-detector.ts 迁移到 src/core/payment/env-detector.ts，以适应新的模块结构。
...
### 其他支付提供商
- WechatProvider：微信支付提供商实现，提供与支付宝类似的支付接口。
- MockProvider：模拟支付提供商，用于开发和测试环境，无需真实支付网关。
