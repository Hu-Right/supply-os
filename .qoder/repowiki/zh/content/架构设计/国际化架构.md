...
## 简介
本文件面向Supply OS的国际化（i18n）体系，系统性阐述以下主题：
...
## 项目结构
国际化相关代码集中在 src/locales 目录，包含上下文状态、类型定义与语言包资源；应用入口与根组件负责注入上下文与提供全局语言环境。

```mermaid
graph TB
A["src/main.tsx"] --> B["src/App.tsx"]
B --> C["src/locales/LocaleContext.tsx"]
C --> D["src/locales/types.ts"]
C --> E["src/locales/en.json"]
C --> F["src/locales/zh.json"]
```
...
## 结论
Supply OS的国际化架构以LocaleContext为核心，结合类型系统与资源文件，实现了类型安全、可扩展且易于维护的多语言支持。通过合理的动态加载与缓存策略，可在保证开发体验的同时兼顾性能与稳定性。遵循本文的最佳实践与工作流程，有助于团队高效推进国际化能力的演进。
