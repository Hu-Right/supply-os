...
- 演示数据：src/data/exhibition-halls.ts、src/data/opportunities.ts、src/data/suppliers.ts
...
B --> D["展厅数据<br/>src/data/exhibition-halls.ts"]
B --> E["商机数据<br/>src/data/opportunities.ts"]
B --> F["供应商数据<br/>src/data/suppliers.ts"]
D --> G["数据聚合入口<br/>src/data/index.ts"]
E --> G
F --> G
```
...
## 架构总览
展览数据管理在应用中以“类型定义 + 演示数据 + 聚合入口 + 页面渲染”的方式组织。类型层保证数据结构一致性；数据层提供样例与扩展点；聚合层统一导出；应用层负责组合与展示。
...
subgraph "数据层"
D1["展厅数据<br/>src/data/exhibition-halls.ts"]
D2["商机数据<br/>src/data/opportunities.ts"]
D3["供应商数据<br/>src/data/suppliers.ts"]
...
O["商机数据<br/>src/data/opportunities.ts"] --> P
H["展厅数据<br/>src/data/exhibition-halls.ts"] --> P
...
Halls["展厅数据<br/>src/data/exhibition-halls.ts"] --> DataIndex
Opps["商机数据<br/>src/data/opportunities.ts"] --> DataIndex
Supp["供应商数据<br/>src/data/suppliers.ts"] --> DataIndex
```
...
## 结论
本技术文档系统梳理了展览数据管理的数据模型、业务规则与关键流程，提供了空间管理、商机工作流、时间调度与冲突检测的设计思路，并给出了可视化、统计分析与报告生成的实现方向。通过类型驱动与数据聚合的架构模式，系统在可扩展性与可维护性方面具备良好的基础。后续建议逐步引入后端服务与真实数据源，完善权限、审计与监控能力，进一步提升系统的稳定性与业务价值。
