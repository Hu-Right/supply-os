...
## 项目结构
仓库采用前端应用 + 轻量服务端（Express）的混合形态。材料相关的数据与类型集中在 src/data 与 src/types 下，服务端入口在 server.ts，构建与运行配置在 package.json。
...
D --> E["HTTP API"]
B --> F["材料数据<br/>materials.ts"]
C --> G["采购类型<br/>procurement.ts"]
C --> H["供应商类型<br/>supplier.ts"]
```
...
- 材料数据模型与分类
  - 材料主数据：包含基础信息、分类、编码、单位、版本、状态等。
  - 分类体系：支持多级分类，便于检索与报表统计。
  - 属性定义：通用属性与扩展属性并存，满足多行业差异。
  - 参考路径：[src/data/materials.ts](file://src/data/materials.ts)、[src/data/index.ts](file://src/data/index.ts)

- 编码规则
  - 规则组成：分类码 + 系列码 + 流水号 + 校验位（可选）。
  - 唯一性：全局唯一，支持前缀隔离不同业务域。
  - 生成策略：自动递增或基于批次/时间戳组合。
  - 校验：长度、字符集、重复性检查。

- 规格参数
  - 结构化字段：长宽高、重量、材质、颜色、公差等。
...
## 结论
Supply OS 材料数据管理系统以清晰的数据模型与分类体系为基础，结合严格的编码规则、规格与质量管理，实现了从入库到出库、盘点与损耗的全链路管控。价格与供应商比价、替代方案与 BOM 管理进一步提升了供应链协同效率。批量导入导出与数据校验保障了数据质量，而使用分析与智能建议则为持续优化提供了数据支撑。
