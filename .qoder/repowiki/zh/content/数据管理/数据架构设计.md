...
## 更新摘要
**变更内容**   
- 数据架构已完全重构，从集中式data.ts文件迁移到模块化数据管理方案
- 新增独立的数据模块文件：exhibition-halls.ts、materials.ts、opportunities.ts、suppliers.ts
- 更新了数据导入导出模式，采用按领域组织的数据结构
- 重新设计了数据聚合入口，提供更清晰的模块边界和依赖关系
...
**更新** 本次更新反映了数据架构的完全重构，从集中式管理转向模块化数据管理方案。

## 项目结构
仓库采用"前端静态数据 + 类型定义 + 服务端入口"的轻量架构。数据相关代码集中在 src/data 与 src/types 两个目录，现已重构为模块化组织：
- src/data：按领域拆分的独立数据模块（展厅、材料、机会、供应商等），每个模块提供独立的导出接口
...
graph TB
subgraph "模块化数据层"
DIndex["data/index.ts<br/>聚合入口"]
DExhibition["data/exhibition-halls.ts<br/>展厅数据模块"]
DMaterials["data/materials.ts<br/>材料数据模块"]
DOpp["data/opportunities.ts<br/>机会数据模块"]
DSuppliers["data/suppliers.ts<br/>供应商数据模块"]
end
subgraph "类型层"
...
- 领域数据模块（exhibition-halls、materials、opportunities、suppliers）
  - 职责：以只读形式提供业务实体集合，作为演示或初始数据的来源
  - 设计要点：字段与类型严格对齐 types 层；保持幂等与可序列化；独立模块化管理
- 类型定义（types/*）
...
## 架构总览
Supply OS 当前采用"类型驱动 + 模块化静态数据 + 服务入口"的分层架构：
- 类型层（types）：定义所有实体的数据结构、枚举与约束
- 数据层（data）：提供模块化只读数据集，供页面或服务端渲染使用
- 服务层（server）：加载配置、注册路由、启动 HTTP 服务
- 依赖与脚本（package.json）：声明运行时依赖与构建/运行脚本

```mermaid
graph TB
Types["类型层<br/>types/*"] --> Data["模块化数据层<br/>data/*"]
...
- 设计原则：
  ...
  - 可扩展性：新增子域只需在聚合处追加导出
- **更新** 现在支持模块化导入，允许直接导入特定数据模块而非全部数据
...
- 展厅数据模块（exhibition-halls.ts）
  ...
  - 独立性：完全独立的模块，可单独导入和使用
- 材料数据模块（materials.ts）
  - 用途：提供材料清单及元信息
  - 关联类型：对齐 types/index.ts 中通用类型
  - 独立性：完全独立的模块，可单独导入和使用
- 机会数据模块（opportunities.ts）
  - 用途：采购/商机机会集合
  - 关联类型：对齐 types/procurement.ts
  - 独立性：完全独立的模块，可单独导入和使用
- 供应商数据模块（suppliers.ts）
  - 用途：供应商主数据
...
- **更新** 现在支持更灵活的数据导入方式，可以选择性地导入特定数据模块
...
## 结论
当前架构以类型为中心、模块化数据为只读、服务为装配点的分层设计，具备清晰的职责边界与良好的可扩展性。模块化重构后，系统获得了更好的可维护性和灵活性。建议在后续演进中引入统一数据访问层、索引与导出能力，并完善版本管理与迁移流程，以提升系统的稳定性与可维护性。
