...
## 简介
本技术文档面向 Supply OS 的国际化（i18n）子系统，聚焦多语言支持的整体架构与实现细节。经过重大架构升级，系统已从自定义本地化方案迁移至react-i18n框架，提供更强大的国际化功能和更好的开发体验。文档重点包括：
- react-i18n框架的集成与配置
...
## 项目结构
国际化相关代码集中在 src/locales 目录下，包含基于react-i18n的重构上下文、类型定义与语言包资源；应用入口与根组件负责初始化与注入i18n配置。

```mermaid
graph TB
A["main.tsx"] --> B["App.tsx"]
B --> C["LocaleContext.tsx (react-i18n集成)"]
C --> D["en.json"]
C --> E["zh.json"]
C --> F["types.ts"]
C --> G["i18n配置"]
...
## 核心组件
- **重构后的LocaleContext**：基于react-i18n框架提供当前语言、切换语言的能力，并维护已加载的语言包缓存，支持更丰富的国际化功能。
- **语言包（en.json、zh.json）**：以 JSON 形式组织翻译键值对，按模块或页面维度分层，兼容react-i18n格式规范。
- **types.ts**：为翻译键与值提供 TypeScript 类型约束，确保编译期安全。
- **i18n配置**：集成react-i18n的核心配置，包括语言检测、回退策略、插件配置等。
...
## 架构总览
整体采用基于react-i18n框架的企业级国际化方案。应用启动时由根组件配置i18n实例，子组件通过Hook获取t函数进行文本渲染；切换语言时利用框架内置的状态管理触发重渲染，支持更复杂的国际化场景。

```mermaid
sequenceDiagram
participant U as "用户"
participant UI as "业务组件"
participant I18n as "react-i18n核心"
...
**更新** 完全重构以集成react-i18n框架，提供更强大的国际化功能

- **职责**
  - 基于react-i18n维护当前locale与已加载语言包缓存
...
App --> Ctx["LocaleContext.tsx (react-i18n)"]
Ctx --> Types["types.ts"]
Ctx --> En["en.json"]
Ctx --> Zh["zh.json"]
Ctx --> I18nCore["react-i18n核心"]
Ctx --> Plugins["插件系统"]
```
...
## 结论
经过重大架构升级，本国际化方案已成功迁移至react-i18n框架，以企业级的国际化能力为核心，结合静态 JSON 语言包与按需加载，实现了强大、可扩展的多语言支持。通过严格的键类型约束、智能回退策略和丰富的插件生态，保障了优秀的开发体验与运行稳定性。新架构支持更复杂的国际化场景，包括复数处理、HTML内容、条件渲染等高级功能，能够满足更大规模的业务需求。
