...
- **幂等的另一半：`period_starts_at` 必须是确定值**。`uk_pool` 含周期起点，而该列 `DEFAULT CURRENT_TIMESTAMP` 是逐条求值的——开池 SQL 若直接用 `NOW()`，同一池跨秒重发就不撞唯一键，而是另开一行满额池（真库实测：连调两次 `quota_total` 由 3 变 6）。放到支付链路上就是回调重试翻倍发额度、已耗尽用户被白送一份新池。因此 `openQuotaPool` 的起点由 `period` 推导：`monthly`→本月月首、`yearly`→本年年初、`none`→该订阅的 `started_at`（普通用户池用 `1970-01-01` 哨兵表示终身）；显式传入的起点优先，供将来周期重置推进行号。
...
- 懒物化允许（首次消费时按矩阵值开池行），但幂等**不是 `uk_pool` 白送的**：只有周期起点是确定值时它才拦得住重复开池（见 §5 「幂等的另一半」条），物化口径由服务层统一封装。

---

*最后更新：2026-09-23（订正两条被实测推翻的承诺：`uk_pool` 不保证幂等——幂等依赖确定周期起点；并补 `openQuotaPool` 的 period 推导规则。此前：表名正式化、逐列注释、普通用户池 NULL 参入唯一约束与 chk_usage 验证）*
