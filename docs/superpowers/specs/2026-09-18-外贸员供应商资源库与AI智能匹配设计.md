...
- **数据复用**：关联平台供应商目录，已有数据自动继承；无记录时在 supplier 表创建基础记录（verify_status='pending'，不进入公共目录），由后台审核后收录
...
外贸员输入公司名称（唯一必填字段）
  ...
  │
  └── 未匹配到 ──→ 在 supplier 表创建基础记录（仅 company 字段，verify_status='pending'）
                    supplier_id 关联到 pool 表
                    引导外贸员填写诊断表（非强制，提升匹配准确度）
                    填写后回填 qualification_id
                    后台审核通过后，该供应商进入公共目录，其他用户也能关联
...
| `enrichRunner` | 后续迭代：后台 AI 数据补全（本期不实现） |
