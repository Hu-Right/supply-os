 ...
 * 只展示按语言选择的标题：中文优先 title_zh；其余语言优先后端附带的界面语言译文
 * title_i18n（fr/ru/es/ar），缺译回退英文（与 fallbackLng: "en" 策略一致）；
...
export const getUnspscOptionLabel = (item: UnspscOption, locale: Locale): string => {
  const title =
    locale === "zh"
      ...
      : item.title_i18n || item.title_en || item.title || item.name || item.title_zh;
