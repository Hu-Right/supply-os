/**
 * 联合国采购招投标能力初筛问卷（手机扫码直达）
 * UN Procurement Capability Screening Form Page
 *
 * @module features/procurement/pages/QualificationFormPage
 * @description 公采系列 — 资质诊断独立表单页，供手机扫码直接访问填写。
 *              复用 training 模块的 CompanyInfoSection 组件（行业联动/认证选择），
 *              去掉 Modal 外壳，移动端优先单列布局。
 *              提交后数据写入 crm_training_registrations 表。
 */

import { useState } from "react";
import { CheckCircle2, Send, ArrowLeft } from "lucide-react";
import { useLocale, pickLocale } from "@/core/i18n";
import CompanyInfoSection, { type CompanyInfoData } from "@/features/training/components/CompanyInfoSection";
    ...
    <div className="min-h-screen" style={{ background: BG_LIGHT }}>
      ...
      <form onSubmit={handleSubmit} className="px-4 pb-10">
        <div className="max-w-lg mx-auto space-y-5">
          ...
          {/* 公司信息表单区块（复用 training 模块的 CompanyInfoSection） */}
