          ...
          <div className="grid md:grid-cols-3 gap-8 md:gap-0 md:divide-x md:divide-[#DCE6F2]">
            {ROLE_COLS.map(({ icon: Icon, titleKey, descKey }) => (
              <div key={titleKey} className="flex items-start gap-4 md:px-8 md:first:pl-0 md:last:pr-0">
                <span className="w-12 h-12 shrink-0 rounded-full bg-[#016E74] flex items-center justify-center">
                  <Icon className="w-6 h-6 text-white" />
                </span>
                <div>
                  <h4 className="text-sm font-black text-[#066364]">{t(titleKey)}</h4>
                  <p className="mt-2 text-xs leading-relaxed text-[#3E5070]">{t(descKey)}</p>
                </div>
              </div>
            ))}
          </div>
              ...
              {team.map((m) => (
                <div key={m.id} className="flex flex-col items-center w-28">
                  <Avatar
                    ...
                    className="w-28 h-28 rounded-full border-2 border-[#287986]/60"
                  />
