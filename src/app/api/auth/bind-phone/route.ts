...
export async function POST(req: NextRequest) {
  ...
  if (!phone || !PHONE_RE.test(phone)) return NextResponse.json({ code: 40011, message: "请输入有效的手机号" }, { status: 400 });
  ...
  if (user.phone) return NextResponse.json({ code: 40031, message: "已绑定手机号，请先解绑或换绑" }, { status: 409 });

  const record = await ctx.user.authRepo.findLatestActiveCode(auth.userKey, "phone_bind", phone);
  ...
  const bound = await ctx.user.usersRepo.bindPhoneIfUnbound(auth.userKey, phone);
  if (!bound) {
    const existingByPhone = await ctx.user.usersRepo.findByPhone(phone);
    if (existingByPhone) return NextResponse.json({ code: 40032, message: "该手机号已被其他用户绑定" }, { status: 409 });
    return NextResponse.json({ code: 40031, message: "已绑定手机号" }, { status: 409 });
  }
