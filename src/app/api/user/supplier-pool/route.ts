...
import { addSupplierToPool } from "@/lib/services/supplier-pool";
...
/** POST: 添加供应商（输入公司名称 → 搜索平台目录 → 匹配或创建；编排下沉 lib/services/supplier-pool） */
export const POST = withRoute(async (req: NextRequest) => {
  ...
  const result = await addSupplierToPool(ctx.dbPool, auth.userId, body.companyName);
  if (!result.ok) {
