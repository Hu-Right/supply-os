/**
 * 商机解锁编排全分支测试（架构评估 P0-T1：executeOpportunityUnlock 无测试）
 *
 * 与公告解锁 executeUnlock 同构的事务编排（审查 F10）：
 * - 快速路径/事务内复查幂等（uk_user_opportunity + ER_DUP_ENTRY）
...
import type { OpportunityUnlockDeps } from "./opportunity-unlock";

vi.mock("server-only", () => ({}));
vi.mock("./unspsc/interest", () => ({ persistUserInterestCodes: vi.fn().mockResolvedValue(undefined) }));

import { persistUserInterestCodes } from "./unspsc/interest";
import { executeOpportunityUnlock, OpportunityUnlockError } from "./opportunity-unlock";
...
describe("executeOpportunityUnlock 免费硬闸（2026-08-30 产品决策）", () => {
  it("free 解锁一律 FREE_LIMIT_REACHED，不写解锁记录", async () => {
    ...
    const result = executeOpportunityUnlock(deps, {
      userKey: "u1", opportunityId: 42, unlockType: "free", price: 0, snapshotJson: "{}",
    });
...
describe("executeOpportunityUnlock 付费解锁配额", () => {
  it("single 持有可用权益：插入解锁 + 配额 +1 + 计数 +1 同事务提交", async () => {
    ...
    // 参数绑定：userKey 子查询占位 + user_key + opportunity_id + unlock_type + price + snapshot
    expect(insertCall![1]).toEqual(["u1", "u1", 42, "single", 99, '{"codes":["123456"]}']);
  ...
  it("subscription 无权益行但有活跃订阅 → 放行且不消耗配额", async () => {
    ...
    const result = await executeOpportunityUnlock(deps, {
      userKey: "u1", opportunityId: 42, unlockType: "subscription", price: 0, snapshotJson: "{}",
    });
  ...
  it("subscription 无权益且无活跃订阅 → PAID_QUOTA_REQUIRED 回滚", async () => {
    ...
    const result = executeOpportunityUnlock(deps, { userKey: "u1", opportunityId: 42, unlockType: "subscription", price: 0, snapshotJson: "{}" });
  ...
  it("single 无任何权益 → PAID_QUOTA_REQUIRED（不兼容订阅放行）", async () => {
    ...
    const result = executeOpportunityUnlock(deps, { userKey: "u1", opportunityId: 42, unlockType: "single", price: 99, snapshotJson: "{}" });
    await expect(result).rejects.toThrow("PAID_QUOTA_REQUIRED");
  });
  ...
  it("guest 用户不写兴趣码", async () => {
    const { deps } = makeEnv({ entitlement: { id: 7 } });
    await executeOpportunityUnlock(deps, { userKey: "guest", opportunityId: 42, unlockType: "single", price: 99, snapshotJson: '{"codes":[]}' });
    expect(persistUserInterestCodes).not.toHaveBeenCalled();
  });
});
