...
/**
 * bcrypt 哈希（所有密码统一使用）
 * bcrypt hash — used for all passwords (registration, reset)
 */
...
/**
 * 密码校验（bcrypt 为唯一算法）。存量 SHA-256 弱哈希账号已停止密码校验，
 * 由登录编排层引导其先走"忘记密码"重置（见 auth-login / EC_PASSWORD_RESET_REQUIRED）。
 */
