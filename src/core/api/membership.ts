/**
 * 会员 API — 全站唯一封装（N5 归属归正 → ARCH-P1b 层级归正）
 * Membership API — Single Source of Truth
 *
 * @module core/api/membership
 * @description ARCH-P1b（2026-08-31）：从 features/membership/api.ts 迁至 core/api/membership.ts，
 *              消除 procurement/auth → membership 跨 feature 依赖。
